            ...
            {resources.map((res, i) => (
               <div key={res.category}>
                  <SketchReveal delay={i * 0.1}>
                     <h3 className="font-cormorant text-2xl font-light text-[#FB7F05] mb-6">{res.category}</h3>
                     <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {res.files.map((file, j) => (
                           <a key={file} href="#" className="flex items-center gap-4 p-4 border border-[rgba(74,74,94,0.1)] hover:border-[#d4a017] group transition-colors">
                              ...
                              <span className="font-inter text-sm text-[#4a4a5e] group-hover:text-[#FB7F05]">{file}</span>
                           </a>
                        ))}
                     </div>
                  </SketchReveal>
