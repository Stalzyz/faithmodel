      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-8 pb-20 border-b border-[rgba(74,74,94,0.08)]">
        <SketchReveal>
          ...
          <h1 className="font-cormorant text-[clamp(3rem,7vw,6rem)] font-light text-[#FB7F05] leading-[1.08] tracking-tight max-w-4xl">
            Careers at<br /><em className="text-[#d4a017] not-italic">Faith Model</em>
          </h1>
        </SketchReveal>
      </div>
         ...
         <div className="mt-12 space-y-4 max-w-4xl">
            {jobs.map((job, i) => (
               <SketchReveal key={job.title} delay={i * 0.05}>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 border border-[rgba(74,74,94,0.1)] hover:border-[#d4a017] transition-colors group">
                     <div>
                        <h3 className="font-cormorant text-2xl font-semibold text-[#FB7F05] mb-2">{job.title}</h3>
                        <div className="flex gap-4 font-poppins text-xs font-semibold text-[#4a4a5e] uppercase tracking-wider">
                           <span>{job.category}</span>
                           <span className="text-[#d4a017]">{job.type}</span>
                        </div>
                     </div>
                     <a href="mailto:careers@faithmodelschool.edu.in" className="mt-4 sm:mt-0 font-poppins text-xs font-semibold text-[#FB7F05] px-6 py-3 bg-[rgba(74,74,94,0.05)] group-hover:bg-[#d4a017] transition-colors shrink-0 text-center">Apply Now</a>
                  </div>
               </SketchReveal>
            ))}
         </div>
