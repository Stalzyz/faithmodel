        ...
        {data.events?.map((item: any, i: number) => (
          <SketchReveal key={i} delay={i * 0.1}>
            <div className="relative flex items-center justify-between md:justify-normal md:even:flex-row-reverse group">
              <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-[#fefcf3] bg-[#FB7F05] text-[#1a1a2e] shadow shrink-0 md:order-1 md:group-even:-ml-5 md:group-odd:-mr-5 z-10" />
              <div className="w-[calc(100%-3rem)] md:w-[calc(50%-2.5rem)] p-6 border border-[rgba(74,74,94,0.1)] rounded-sm bg-white shadow-sm hover:border-[#FB7F05] transition-colors">
